...
> & {
  Top: ({ children }: { children: React.ReactNode }) => React.ReactNode
  Bottom: ({ children }: { children: React.ReactNode }) => React.ReactNode
}

const SelectContentTop = ({ children }: { children: React.ReactNode }) => (
  <div>{children}</div>
)

const SelectContentBottom = ({ children }: { children: React.ReactNode }) => (
  <div>{children}</div>
)
...
>(({ className, children, position = "popper", ...props }, ref) => {
  // Split children into top, bottom and main
  const childrenArray = React.Children.toArray(children)
  const childrenTop = childrenArray.filter(
    (child) => React.isValidElement(child) && child.type === SelectContentTop
  )

  const childrenBottom = childrenArray.filter(
    (child) => React.isValidElement(child) && child.type === SelectContentBottom
  )

  const childrenMain = childrenArray.filter(
    (child) =>
      React.isValidElement(child) &&
      child.type !== SelectContentTop &&
      child.type !== SelectContentBottom
  )
  // ------------------------------------
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        {...props}
      >
        {!!childrenTop.length && childrenTop}
        ...
        {!!childrenBottom.length && childrenBottom}
      </SelectPrimitive.Content>
    </SelectPrimitive.Portal>
  )
...
SelectContent.Top = SelectContentTop

SelectContent.Bottom = SelectContentBottom
