...
// Example with status tags render: (item) => ({ type: 'tagList', value: { type:
'status', tags: [ { type: 'status', variant: 'positive', text: 'Active' }, {
type: 'status', variant: 'warning', text: 'Needs Review' } ] } }) `, }} />
